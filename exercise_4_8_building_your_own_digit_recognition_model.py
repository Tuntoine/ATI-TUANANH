...
Original file is located at
    https://colab.research.google.com/drive/12bm-eNCxiAKIN42Teih9T_GLXbwLqAJG
...
# Import EarlyStopping and other necessary modules
...
predictors_normalized = predictors / 255.0
pred_data_normalized = pred_data / 255.0

def get_improved_model(input_shape):
  # Set up the model
  model = Sequential()
  
  # Lớp ẩn đầu tiên rộng hơn
  model.add(Dense(128, activation='relu', input_shape=input_shape))
  # Thêm Dropout để chính quy hóa
  model.add(Dropout(0.3))
  
  # Lớp ẩn thứ hai rộng hơn
  model.add(Dense(64, activation='relu'))
  # Thêm Dropout
  model.add(Dropout(0.3))
  
  # Lớp đầu ra không đổi
  model.add(Dense(10, activation='softmax'))
  return model

# Khởi tạo, biên dịch và huấn luyện mô hình đã cải tiến
improved_model = get_improved_model(input_shape)
...
improved_model_training = improved_model.fit(predictors_normalized, 
                                             target, 
                                             epochs=20, 
                                             validation_split=0.3, 
                                             callbacks=[early_stopping_monitor])

# Lưu mô hình đã cải tiến
improved_model.save('myImprovedModel.keras')

# Tải lại mô hình đã cải tiến và thực hiện dự đoán
myImprovedModel = load_model('myImprovedModel.keras')
# Thực hiện dự đoán trên dữ liệu dự đoán đã được chuẩn hóa
predictions = myImprovedModel.predict(pred_data_normalized)
prediction_classes = np.argmax(predictions, axis=1)
print("\nKết quả dự đoán từ mô hình đã cải tiến:")
print(prediction_classes)

# Hiển thị 100 ảnh trong mảng pred_data với kết quả dự đoán mới
print("\nHiển thị hình ảnh với nhãn dự đoán từ mô hình đã cải tiến:")
